...
function App() {
  ...
  // Consolida cantidades cuando el producto ya existe en el carrito.
      ...
      if (productoEnCarrito) {
        return carritoActual.map((productoEnLista) =>
          productoEnLista.id === producto.id
            ? {
                ...productoEnLista,
                cantidad: productoEnLista.cantidad + cantidad,
              }
            : productoEnLista
        );
      }
    ...
    setCarrito((carritoActual) =>
      carritoActual.map((productoEnLista) =>
        productoEnLista.id === id
          ? { ...productoEnLista, cantidad: productoEnLista.cantidad + 1 }
          : productoEnLista
      )
    );
  };
  ...
  const { totalServicios, totalPrecio } = useMemo(() => {
    // Calcula los totales del carrito para el resumen.
  ...
  return (
    <BrowserRouter>
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-50">
        ...
        <NavBar totalServicios={totalServicios} totalPrecio={totalPrecio} />
        ...
        <main className="mx-auto flex max-w-6xl flex-col gap-10 px-4 pb-14 pt-10">
          <Routes>
            ...
            <Route
              path="/item/:itemId"
              element={<ItemDetailContainer onAgregar={agregarAlCarrito} />}
            />
